...
def _():
    ...
    return alt, datetime, defaultdict, json_graph, json_lib, np, nx, pd


@app.cell
def _(mo):
    def data_file(filename):
        """Open a bundled data file.

        Works both locally (`marimo run`, `marimo edit`) and in the browser
        after `marimo export html-wasm`, where the notebook is served over HTTP
        and there is no local filesystem. Returns a text file-like object that
        pandas / json can read directly.
        """
        import io as _io

        _path = str(mo.notebook_location() / "public" / filename)
        if _path.startswith("http"):
            from pyodide.http import open_url

            return open_url(_path)
        with open(_path, "r", encoding="utf-8") as _f:
            return _io.StringIO(_f.read())

    return (data_file,)


@app.cell
def _(data_file, json_graph, json_lib):
    graph_data = json_lib.load(data_file("MC3_graph.json"))
...
@app.cell
def _(data_file, pd):
    df_intents = pd.read_csv(data_file("categories_v2.csv"))
...
@app.cell
def _(data_file, pd):
    plot_df = pd.read_csv(data_file("topic_plot_df.csv"))
...
@app.cell
def _(data_file, pd):
    topics_df = pd.read_csv(data_file("topics_df.csv"), index_col=0)
    _kw_df = pd.read_csv(data_file("topic_keywords.csv"))
